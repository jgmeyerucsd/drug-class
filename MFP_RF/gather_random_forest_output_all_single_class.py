...
if __name__ == '__main__':
    ...
    for class_num in [3, 5, 12]:
        ...
        for index, hyper_id in enumerate(class_num_to_hyper_id[class_num]):
            file_path = 'output_all_single_class/num_class_{}_id_{}_index_{}.out'.format(class_num, hyper_id, index)
        ...
        print('optimal test accuracy: {}'.format(np.mean(test_acc_list)))
        print('optimal test balanced accuracy: {}'.format(np.mean(test_balanced_acc_list)))
        print('optimal test matthews corrcoef: {}'.format(np.mean(test_matthews_corref_list)))
        print('optimal test multiclass ROC-AUC: {}'.format(np.mean(test_roc_list)))
        print('optimal test multiclass PR-AUC: {}'.format(np.mean(test_pr_list)))
        print()